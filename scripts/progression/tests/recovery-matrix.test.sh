...
set -u
export FORWARD_TEST_MODE=1; FWROOT=".forward-test-tmp/recovery-matrix"; export FORWARD_TEST_ROOT="$FWROOT"; rm -rf "$FWROOT"; mkdir -p "$FWROOT"
cd "$(dirname "$0")/../../.."
...
D="$FWROOT/recovmx"; rm -rf "$D"; mkdir -p "$D"
